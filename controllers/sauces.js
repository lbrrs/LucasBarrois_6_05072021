...
exports.usersLike =(req, res, next) => {
  ...
  if (like == 1){  // like de la sauce
    Sauce.updateMany( // update est obselete preference de updateMany
    ...
    Sauce.updateMany(  // update est obselete preference de updateMany
      { _id: sauceId }, // Parametres de recherche pour mettre a jour la valeur des dislike et le tableau des Users qui n'aime pas la sauce
        {
    ...
    Sauce.findOne({ _id: req.params.id })// recherche de la sauce par sont _id dans la bdd par la sauce selectionné par l 'utilisateur
    .then(
      (sauces) => {
      if (sauces.usersDisliked.find(userId => userId === req.body.userId)){ // verification que l'utilisateur est bien dans le tableau des personnes qui n'aime pas la sauce
        Sauce.updateMany(  // update est obselete preference de updateMany
        { _id: sauceId }, 
          {
